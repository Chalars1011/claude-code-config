...
- 美术相关（看图/评图/改图/生图/调色/贴图验收）→ 必走伊莉丝。主控与锐亚禁止自行解码贴图拍板、禁止拿像素统计冒充美术判断、禁止绕过画笔自行生成图。义眼评审不过关不发布。此条为 HARD RULE，2026-08-06 查尔斯拍板。
- 绘画铁律（2026-08-10 查尔斯拍板，取代原省钱铁律）：美术出图一律走画笔（Qwen-Image API），禁止用 Pillow 调色/ps 代替画笔——Pillow 调出来的低水准图不准出现在任何交付里。花钱不是问题，水准是底线。禁止反复试错烧钱（一次到位：先想清描述再生成，义眼验收不过再改）。

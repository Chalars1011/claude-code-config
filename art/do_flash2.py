#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Steam 版 PCK 花屏测试：改 Steam 目录的 card_atlas_2 为全 0xFF"""
...
CUR_PCK = r"D:/steam/steamapps/common/Slay the Spire 2/SlayTheSpire2.pck"
OUT = r"D:/steam/steamapps/common/Slay the Spire 2/SlayTheSpire2.pck.modtest"
AL = 32
...
def main():
    ...
    for pp, off, size in entries:
        if pp == ATLAS_ENTRY:
            with open(CUR_PCK, "rb") as f:
                ...
                orig = f.read(size)
            break
    print(f"Steam PCK ctex: {len(orig)}B, GST2@0: {orig[:4] == b'GST2'}")
    need = 10866240
    new_ctex = orig[:36] + b"\xff" * need + orig[36 + need:]
    ...
    # 替换回去
    shutil.move(OUT, CUR_PCK)
    print("✅ Steam 版 PCK 已替换为花屏版（原版备份在 .pck.modtest.orig）")

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""花屏测试：把 card_atlas_2 数据替换成全 0xFF，验证游戏是否加载我们的 PCK"""
...
def main():
    base, dir_off, entries = read_entries()
    orig = None
    for pp, off, size in entries:
        if pp == ATLAS_ENTRY:
            with open(CUR_PCK, "rb") as f:
                ...
                orig = f.read(size)
            break
    print(f"原 ctex: {len(orig)}B, GST2@0: {orig[:4] == b'GST2'}")
    need = 10866240
    # 花屏数据：保留 GST2 头和尾部，数据全 0xFF
    new_ctex = orig[:36] + b"\xff" * need + orig[36 + need:]
    print(f"花屏版: {len(new_ctex)}B")

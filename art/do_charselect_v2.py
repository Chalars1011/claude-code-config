...
SRC_PCK = r"D:/泯灭之塔/SlayTheSpire2.pck.bak_menu_static_prev"
...
CHAR_PNG = r"D:/泯灭之塔/采纳图/选人/立绘_silent_原生镜像.png"
...
def main():
    ...
    with open(SRC_PCK, "rb") as f:
        ...
        for i in range(n):
            ...
            elif path == SKEL_TARGET:
                with open(SKEL_FILE, 'rb') as fsk:
                    data = fsk.read()
                changed.append(path)
                print(f"  🔄 噬影蒙皮(原版): {path.split('/')[-1]}")
            elif path == TSCN_TARGET:
                with open(TSCN_FILE, 'rb') as ft:
                    data = ft.read()
                changed.append(path)
                print(f"  🔄 选人场景(Sprite2D): {path.split('/')[-1]}")
            pbytes = path.encode("utf-8")
            ppl = len(pbytes)
            while ppl % 4 != 0:
                ppl += 1
            entries.append((path, pbytes, ppl, data, ef))
    print(f"总条目: {len(entries)} | 已替换: {len(changed)} | 新增: 13")
    assert len(changed) == 3, "替换数不对（应3个）！"

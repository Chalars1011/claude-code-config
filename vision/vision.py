...
DEFAULT_PROMPT = (
    "【客观描述模式】只描述画面中确实可见的内容，禁止任何联想、推测、评价。"
    "1) 主体：画面中有什么（人物/物体），其外观（颜色、形态、姿态）、在画面中的位置（左/右/上/下/中央）"
    "2) 颜色与明暗：主色调、亮部/暗部分布"
    "3) 可见文字：如有，原样引用"
    "4) 背景与其他元素"
    "铁律：不猜图片来自哪里、是什么作品、什么含义；不做美丑好坏评价；看不清就说看不清；"
    "禁止引用任何游戏/作品/角色名称（除非画面文字里明确写着）。用中文，按条目输出。"
)
